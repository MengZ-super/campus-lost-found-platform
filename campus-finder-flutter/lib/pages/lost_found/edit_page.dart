...
class _EditLostFoundPageState extends State<EditLostFoundPage> {
  ...
  Future<void> _loadData() async {
    try {
      final lostFoundService = context.read<LostFoundService>();
      final categoryService = context.read<CategoryService>();

      final results = await Future.wait([
        lostFoundService.getDetail(widget.id),
        categoryService.getList(),
      ]);

      final item = results[0] as LostFoundItem;
      ...
      _featuresController.text = item.features ?? '';
      _locationController.text = item.location;
      _campus = item.campus;
      _happenedTime = item.happenedTime;
      _latitude = item.latitude;
      _longitude = item.longitude;
      _imageUrls = item.images ?? [];
  ...
  Future<void> _submit() async {
    ...
    try {
      final request = UpdateLostFoundRequest(
      ...
      await service.update(widget.id, request);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('保存成功')),
        );
        GoRouter.of(context).go('/lost-found/${widget.id}');
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget _buildTypeDisplay(ThemeData theme) {
    final isLost = _itemType == 'lost';
